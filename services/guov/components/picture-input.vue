 ...
 */
  ...
  methods: {
    ...
    async preloadImage (source, options) {
      ...
      try {
        ...
        if(this.request) {
            imageBlob = await this.request(source, void 0, {not_parse_url: true, no_headers: true, config: {responseType:'blob'}});
            imageBlob = imageBlob.data;
        }
        else {
          imageBlob = await fetch(source, {
              method: 'GET',
              mode: 'cors',
              headers: headers
          });
...
/*       if(this.request) {
          let image = await this.request(source, void 0, {config: {responseType:'blob'}});
          console.log(image);
      }
      else {
